...
exports.createPayment = async (req, res) => {
  ...
  try {
    ...
    console.log(`Calculated total: ${totalAmount}, Order total: ${parseFloat(order.total_amount).toFixed(2)}`);

    // Kiểm tra nếu tổng số tiền của các mục khớp với tổng số tiền của đơn hàng
  /*  if (parseFloat(totalAmount) !== parseFloat(order.total_amount).toFixed(2)) {
      console.log(`Mismatch detected: Calculated total ${totalAmount}, Order total ${parseFloat(order.total_amount).toFixed(2)}`);
